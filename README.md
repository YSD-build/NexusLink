...
![Version](https://img.shields.io/badge/version-v0.3.6-4f6ef7)
...
---
## 📦 下载安装
所有资产发布在 **[v0.3.6 Release](https://github.com/YSD-build/NexusLink/releases/tag/v0.3.6)**。
### 🖥️ 服务端（6 架构）
| 架构 | 文件名 | 适用设备 |
|------|--------|----------|
| linux-x86_64 | `nexuslink-server-v0.3.6-linux-x86_64` | PC、云服务器、虚拟机 |
| **linux-arm64** | `nexuslink-server-v0.3.6-linux-armv8` | ✅ ARM 服务器、树莓派 4/5 |
| linux-armv7 | `nexuslink-server-v0.3.6-linux-armv7` | 路由器、树莓派 2/3 |
| linux-armv6 | `nexuslink-server-v0.3.6-linux-armv6` | 旧嵌入式设备 |
| windows-x86_64 | `nexuslink-server-v0.3.6-windows-x86_64.exe` | Windows PC (x64) |
| windows-arm64 | `nexuslink-server-v0.3.6-windows-arm64.exe` | Windows ARM (骁龙 X) |
### 📱 客户端（6 版本）
| 架构 | 文件名 | 适用设备 |
|------|--------|----------|
| **android-arm64** | `nexuslink-client-v0.3.6-android-arm64` | ✅ 骁龙、天玑、绝大多数安卓手机（Termux 无 Root） |
| linux-x86_64 | `nexuslink-client-v0.3.6-linux-x86_64` | PC、虚拟机 |
| linux-armv8 | `nexuslink-client-v0.3.6-linux-armv8` | ARM 服务器、树莓派 |
| linux-armv7 | `nexuslink-client-v0.3.6-linux-armv7` | 路由器 |
| linux-armv6 | `nexuslink-client-v0.3.6-linux-armv6` | 旧嵌入式设备 |
| windows-x86_64 | `nexuslink-client-v0.3.6-windows-x86_64.exe` | Windows PC |
### 🌐 Web 面板
`nexuslink-web-panel-v0.3.6.zip` — 独立 Node.js 版管理面板（已内置 x86_64 二进制）。服务端二进制本身也已内置面板，直接访问 `web_addr:web_port` 即可。
### 🐳 Docker 部署
...
```
> 版本化镜像：`ghcr.io/ysd-build/nexuslink:0.3.6`（`v*` tag 自动触发构建发布）。默认配置见 `docker/server.yaml`。
---
## 🚀 快速开始
### 1️⃣ 服务端部署（公网服务器）
```bash
# 以 ARM64 为例
wget https://github.com/YSD-build/NexusLink/releases/download/v0.3.6/nexuslink-server-v0.3.6-linux-armv8
chmod +x nexuslink-server-v0.3.6-linux-armv8
```
**`server.yaml`：**
```yaml
bind_addr: 0.0.0.0
...
```
**运行：** `./nexuslink-server-v0.3.6-linux-armv8 -c server.yaml`
正常输出：
```
...
proxies:
  ...
  game_udp:
    ...
    localport: 9000
```
**运行（Linux）：** `./nexuslink-client-v0.3.6-linux-armv8 -c client.yaml`
正常输出：
```
...
2. 下载 `android-arm64` 版本，Termux 中运行：
   ```bash
   pkg install wget
   wget https://github.com/YSD-build/NexusLink/releases/download/v0.3.6/nexuslink-client-v0.3.6-android-arm64
   chmod +x nexuslink-client-v0.3.6-android-arm64
   ./nexuslink-client-v0.3.6-android-arm64 -c client.yaml
   ```
---
## ⚙️ 配置详解
### 服务端 `server.yaml`
